class Garden {
    ...
    generateReport() {
        ...
        for (const storagePlant of this.storage) {
            thirdLine += `${storagePlant.plantName} (${storagePlant.quantity}), `
        }

        return `${firstLine}\n${secondLine.substring(-2, secondLine.length - 2)}\n${thirdLine.substring(-2, thirdLine.length - 2)}`
    }
}
